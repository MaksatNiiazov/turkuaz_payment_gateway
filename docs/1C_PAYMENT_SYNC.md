...
Событие с ошибкой переводится в статус `error` и больше не возвращается методом
`pending`, пока администратор не исправит причину и не выполнит reset. После
успеха событие из `pending` исчезает.

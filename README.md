...
Первая сохраненная транзакция со статусом `paid` и стабильным `invoiceId`
автоматически попадает в отдельную очередь 1С. Остальные QR того же счета
отменяются у провайдера, а запоздалые повторные `paid` callback'и помечаются
как дубли и не попадают в очереди 1С/Tiger. Доставка в Tiger и доставка в 1С
подтверждаются независимо.
